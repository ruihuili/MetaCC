...
def compute_ccas(sigma_xx, sigma_xy, sigma_yx, sigma_yy, epsilon,
  ...
  if verbose:
    ...
    # print(inv_xx, inv_yy)
    print("before sqrt", np.any(inv_xx<0), np.any(inv_yy<0))
    print("np.where(inv_xx < 0)", np.where(inv_xx < 0))

    ids_xx = np.where(inv_xx < 0)
    ids_yy = np.where(inv_xx < 0)

    for i in ids_xx: inv_xx[i] = -inv_xx[i]#1e-10
    # print("np.where(inv_xx < 0)", np.where(inv_xx < 0))
    for i in ids_yy: inv_yy[i] = -inv_xx[i] #1e-10

    print("before sqrt number of values < 0", len(np.where(inv_xx<0)), len(np.where(inv_yy<0)))
    # print("before sqrt", np.isnan(inv_yy).any(),np.isnan(inv_yy).any())
  ...
  if verbose:
    ...
    print("here", np.isinf(arr).any(), np.isnan(arr).any(), np.isinf(arr).all(), np.isnan(arr).all())
