...
def t_dist3(in_array1, in_array2):
    """
    ...
    """
    distance = 2.0 *math.log( ((in_array1 - in_array2)*(in_array1 - in_array2)).sum() + 3)

    return distance

def t_dist5(in_array1, in_array2):
    """
    Computes the squared euclidean distance between two NumPy arrays
    ...
    distance : float
        Squared Euclidean distance between two input arrays.
    """
    distance = 3.0* math.log( ((in_array1 - in_array2)*(in_array1 - in_array2)).sum() + 5)
...
#    print('here')
    ...
    for i in range(int(number_inbits/k)): # Loop through all input bits
        #print('** message input is', message_bits)
        #print('range(whatever) is', range(int(number_inbits/k)))
        #print("number inbits ", number_inbits, " i ", i," k ", k, " inbits size ", np.size(inbits), " inbits shape ", np.shape(inbits), "/n inbits ", inbits, "/n")
        #print("inbits[i*k:(i+1)*k]", inbits[i*k:(i+1)*k] )
...
def _acs_traceback(r_codeword, trellis, decoding_type,
    ...
    for state_num in range(current_number_states):
        ...
        for i in range(number_found):
            ...
            elif decoding_type == 'tdist3':
                i_codeword_array = 2*i_codeword_array - 1
                #branch_metric = euclid_dist(r_codeword, i_codeword_array)
                branch_metric = t_dist3(r_codeword, i_codeword_array)

            elif decoding_type == 'tdist5':
                i_codeword_array = 2*i_codeword_array - 1
                #branch_metric = euclid_dist(r_codeword, i_codeword_array)
                branch_metric = t_dist5(r_codeword, i_codeword_array)
...
def viterbi_decode(coded_bits, trellis, tb_depth=None, decoding_type='hard'):
    ...
    for t in range(1, int((L+total_memory+total_memory%k)/k) + 1):
        ...
        if t <= L:
            ...
            elif decoding_type == 'tdist3' or decoding_type == 'tdist5':
                r_codeword[:] = 0
                r_codeword = 2*r_codeword - 1
            else:
                pass
