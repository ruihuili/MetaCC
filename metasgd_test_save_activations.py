...
def main(args, device):
    ...
    msgd = l2l.algorithms.MetaSGD(model, lr=fast_lr, first_order=False)
    load_dict = torch.load(model_path)
    msgd.load_state_dict(load_dict)
    opt = optim.Adam(msgd.parameters(), meta_lr)
    ...
    # for task in range(10*50): #awgn snr 0
    # for task in range(25*50): #mem center
    # for task in range(34*50):# mp center 34*50
    for task in range(44*50): #bursty center 
        ...
        if args.save_adapt_acts == "before":
        ...
        elif args.save_adapt_acts == "after":      
    ...
    if args.save_adapt_acts == "before":
        ...
        with open(os.path.join("activations/targ_bursty_center/", args.name + "_before.pickle"), 'wb') as f:
        ...
        with open(os.path.join("activations/targ_bursty_center/", args.name +"_after.pickle"), 'wb') as f:
